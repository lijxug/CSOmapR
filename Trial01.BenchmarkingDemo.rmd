...
time_all_tbl = bind_rows(
  ...
  tibble(
    ...
    seed = 1:5,
    time_elapsed = c(
      14.081, 14.656, 13.509, 14.413, 13.238
    )
  )
)
...
```{r}
demoMatlab_lst = list()
data_dir = "/home/jason/git/CSOmapR/results/matlab.new/"
...
for(dir2read in demoMatlab_dirs){
  ...
  pvalue_raw_tbl = read_tsv(paste0(dir2read, "/result/", sample_name, "_pvalues.txt"))
  pvalue_mt = pvalue_raw_tbl[, -c(1, ncol(pvalue_raw_tbl))] %>% as.matrix()
  rownames(pvalue_mt) = pvalue_raw_tbl$`p-value`
  pvalue_tbl = pvalue_mt  %>% melt(varnames = c("cluster1", "cluster2"), value.name = "p.value") %>% 
    unite(col = "cluster_pair", cluster1, cluster2, sep = "---", remove = T)
  
  qvalue_raw_tbl = read_tsv(paste0(dir2read, "/result/", sample_name, "_statistics.txt"))
  qvalue_mt = qvalue_raw_tbl[,-c(1, ncol(qvalue_raw_tbl))] %>% as.matrix()
  rownames(qvalue_mt) = qvalue_raw_tbl$`q-value`
  qvalue_tbl = qvalue_mt  %>% melt(varnames = c("cluster1", "cluster2"),
                                   value.name = "q.value") %>%
    unite(col = "cluster_pair",
          cluster1,
          cluster2,
          sep = "---",
          remove = T)
  
  signif_tbl2save = left_join(pvalue_tbl, qvalue_tbl, by = "cluster_pair")
  
  # rerun using my implementation
  ...
  t_signif_res = getSignificance(coords, labels = labels)
  ...
  demoMatlab_lst[[paste0(sample_name, "_matlab_Rsignif")]] = list(
    coords = coords_tbl,
    signif_res = t_signif_res,
    cellinfo = cellinfo_tbl
  )
}
...
for(chosen_sample in names(demoAll_lst)){
  signif_tbl = 
    bind_rows(
      signif_tbl, 
      demoAll_lst[[chosen_sample]]$signif_res$pvalue %>% melt(varnames = c("cluster1", "cluster2"), value.name = "p.value") %>%  
        dplyr::mutate(sample_name = chosen_sample)
    )
}

plt_p_mt = signif_tbl %>% 
  mutate(cluster_pair = paste2columns(cluster1, cluster2)) %>% 
  dplyr::select(cluster_pair, sample_name, p.value) %>% 
  distinct() %>% 
  acast(cluster_pair ~ sample_name, value.var = "p.value")
plt_p_mt = -log10(plt_p_mt + 1e-3)


col_fun = circlize::colorRamp2(c(0, 1.3, 3), c("blue", "white", "red"))
ComplexHeatmap::Heatmap(t(plt_p_mt), name = "-log10(p.value)", col = col_fun)
...
for(chosen_sample in names(demoAll_lst)){
  signif_tbl = 
    bind_rows(
      signif_tbl, 
      demoAll_lst[[chosen_sample]]$signif_res$qvalue %>% melt(varnames = c("cluster1", "cluster2"), value.name = "q.value") %>%
        dplyr::mutate(sample_name = chosen_sample)
    )
}
...
plt_q_mt = signif_tbl %>% 
  mutate(cluster_pair = paste2columns(cluster1, cluster2)) %>% 
  dplyr::select(cluster_pair, sample_name, q.value) %>% 
  distinct() %>% 
  acast(cluster_pair ~ sample_name, value.var = "q.value")

plt_q_mt = -log10(plt_q_mt + 1e-3)

col_fun = circlize::colorRamp2(c(0, 1.3, 3), c("blue", "white", "red"))
ComplexHeatmap::Heatmap(t(plt_q_mt), name = "-log10(q.value)", col = col_fun)
```
