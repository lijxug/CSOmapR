...
// [[Rcpp::export]]
Rcpp::List Rtsne_cpp(
  NumericVector X, int N, int D, NumericVector Y, int no_dims, double perplexity, double theta, int rand_seed,
  ...
  int load_affinities, int perplexity_list_length, NumericVector perplexity_list, double df,
  ...
  double *pX = X.begin();
  double *pY = Y.begin();
  double *pPerplexity_lst = perplexity_list.begin();
  // parse args
  ...
  if(skip_random_init == false){
    ...
    double *pY = (double*) malloc(N * no_dims * sizeof(double));
  }
  
  // Run tsne
  if (verbose) {printf("Initiate a TSNE object\n");}
  TSNE* tsne = new TSNE();
  // Now fire up the SNE implementation
  double* costs = (double*) calloc(max_iter, sizeof(double));
  if(costs == NULL) { printf("Memory allocation failed!\n"); exit(1); }
  int error_code = 0;
  error_code = tsne->run(pX, N, D, pY, no_dims, perplexity, theta, rand_seed, skip_random_init, max_iter, 
                         ...
                         perplexity_list_length, pPerplexity_lst, df, max_step_norm, verbose);
  ...
  // arrange outputs
  return Rcpp::List::create(Rcpp::_["Y"]=*pY, 
                            // Rcpp::_["costs"]=as<NumericVector>(*costs),
